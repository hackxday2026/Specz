  ...
  try {
    ...
    const items = await page.$$eval(".product-thumb, .product-layout", els =>
      els.map(el => ({
        ...
        priceText: el.querySelector(".price-view-setion, .price")?.innerText || ""
    ...
    console.error("Evo Store search error:", e.message);
    return [];
  } finally {
    await browser.close();
  }
...
async function scrapeEvoProduct(url) {
  ...
  try {
    ...
    console.error("Evo Store scraper error:", error.message);
    return null;
  } finally {
    await browser.close();
  }
